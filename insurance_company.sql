...
CREATE TABLE IF NOT EXISTS `insurance_type` (
  ...
  `deductible_value` DECIMAL(50) NOT NULL,
  `premium_value` DECIMAL(50) NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `insurance_plan` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `premium_total` DECIMAL(50) NOT NULL,
  `deductible_total` DECIMAL(50) NOT NULL,
  `type_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_insurance_plan_type_id_to_insurance_type_id_idx` (`type_id` ASC),
  CONSTRAINT `fk_insurance_plan_type_id_to_insurance_type_id`
    FOREIGN KEY (`type_id`)
